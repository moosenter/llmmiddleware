...
class Milvus_VectorStore(VannaBase):
    ...
    def __init__(self, config=None):
        ...
        if "milvus_client" in config:
            connections.connect(uri=config["milvus_client"], user=user, password=password, token=token)
            self.milvus_client = MilvusClient(uri=config["milvus_client"])
        else:
            connections.connect(uri=DEFAULT_MILVUS_URI, user=user, password=password, token=token)
    ...
    def _create_sql_collection(self, name: str):
        ...
        # print(utility.has_collection(collection_name=name))
